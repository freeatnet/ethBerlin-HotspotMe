...
const micropay = async (amount, recipient) => {
  ...
  console.log(`Got balance of ${balance.channel.token.format()}`)
  if (balance.channel.token.wad.lt(parseEther(amount))) {
  ...
  if (await channel.recipientNeedsCollateral(recipient, payment)) {
    try {
