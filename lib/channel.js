...
const getToken = async () => {
  ...
  const wallet = eth.Wallet.fromMnemonic(await getMnemonic()).connect(eth.providers.JsonRpcProvider(ethUrl))
  return new Contract(channel.opts.tokenAddress, tokenArtifacts.abi, wallet);
}

const getSwapRate = async () => {
  const ethprovider = eth.providers.getDefaultProvider();
  const medianizer = new ethers.Contract(MEDIANIZER_ADDRESS, medianizerAbi, ethprovider);
  const swapRate = eth.utils.formatEther((await this.medianizer.peek())[0]);
  console.log(`Got swap rate: ${swapRate}`)
  return swapRate;
}

const getBalance = async () => {
  const ethprovider = eth.providers.JsonRpcProvider(ethUrl);
