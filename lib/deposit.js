...
const depositAndSwap = async (balance, channel, swapRate, token, setState) => {
  ...
  if (ethAmount.gt(DUST)) {
    ...
    setState({ balance: currentBalance })
  } else {
    console.log(`Not enough on-chain eth to cover gas money`);
  }
  ...
  if (weiToSwap.gt(DUST)) {
    ...
    console.log(`Not enough eth to swap for dai`);
  }
}
